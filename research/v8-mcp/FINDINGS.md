...
3. **Small.** A one-time backfill with Haiku 4.5 through the Batch API costs about
   ...
   conversations. For 10,000 conversations, Sonnet 5 costs about $70 and Opus 5.5 about
   $137. The full table is below.
...
12 transcripts per model, all JSON valid (36/36). Raw records are in `results/cost.jsonl`, and the model output is in `results/cost-model.md`.
...
- **Thinking.** Claude Code runs Haiku 4.5 with thinking even at low effort. Visible
  output averaged under 200 tokens, but thinking averaged over 1,600. Sonnet 5 and
  Opus 5.5 reported no thinking tokens at the same setting. A batch job would
...
- **Tokenizer.** Sonnet 5 and Opus 5.5 produce about 40% more tokens than Haiku 4.5
  for the same text (1.30 vs 0.93 per Llama-2 token). That matches Anthropic's note on the newer tokenizer, and it is
...
- **Quality was checked by eye, not scored.** All outputs parsed as JSON. Haiku found
  ...
  - Its `confidence` sat at 90–98 for everything, so it cannot drive
    `AUTO_APPROVE_AT` as-is. Sonnet (80–90) and Opus (70–95) were no better. None of
    them rated a unit conversion lower than a real decision.
