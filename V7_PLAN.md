...
**Status after PR 21:** 2, 3, 5, 6 and 7 hold across every view; 4 is enforced globally; 1 is at 20 UI colors against a target of about a dozen, the remainder named in row 21 (since reduced to 13, below).

**Follow-up (PR 22, after the sequence):** the screenshot baseline now really runs
with reduced motion (the config's `reducedMotion` was silently ignored; it has to go
through `contextOptions`), and Rewind's motion path keeps its own `rewind-motion`
capture. Six of the seven one-off UI colors are folded into the core set (pink and
violet to purple, orange, yellow and cyan to amber, redLight to red; navy, used only
for an `<option>` and the error box, to `bg2`): principle 1 now stands at 13 UI
colors, the background ramp (4), white, black, gold, amber, green, red, blue, purple
and slate, the one neutral the core set was missing. The phone conversation page no
longer runs 23px past the screen.
