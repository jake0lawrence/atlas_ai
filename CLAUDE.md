...
| `src/styles/shared.js` | Inline-style objects shared across views (`row`, `rowTight`, `container`, `screen(mobile)`, `title(mobile)`, `lede(mobile)`). Extend with a spread: `{ ...lede(mobile), marginTop: 6 }`. `src/styles/__tests__/unused.test.js` fails on any color, keyframe or shared style nothing uses: delete it rather than keep it. |
...
| `src/privacy.js` | Privacy mode (#86): `ALIASES` gives every name in `ENTITIES` (fixtures) a typed, stable stand-in ("Employer A", "Person 1"); `aliasText` swaps them in any string and bands dollar figures ("$500–$1k"), `leaks` finds any name or exact amount left, `privatePath`/`realPath` spell topic URLs with the stand-ins and back. Decisions on its open questions: V8_PLAN.md. `components/PrivacyShield.jsx` applies it to everything on screen (text nodes and labeling attributes, before paint); exports, the clipboard and palette search call `aliasText` themselves. Toggle: Alt+Shift+P, the header's ◐ button, the palette, or the phone menu. |
| `src/mcp/readTools.js` | The v8 MCP server's read tools (`atlas_search`, `atlas_topic`, `atlas_decisions`, `atlas_drift`) as pure functions over the fixtures, honoring privacy mode. Fixture imports carry `.js` so plain Node can load them. |
| `src/mcp/server.js` | The MCP server over those tools (`buildServer`) and `handleMcpRequest` (web `Request` in, `Response` out; stateless Streamable HTTP). Hosted by `api/mcp.js`, a Vercel function at `/api/mcp` (`?privacy=on` for the stand-ins); run locally by `research/v8-mcp/read-server.mjs` (stdio or `--http`). `read-smoke.mjs [url]` checks either over MCP. Its tests run in Node (`// @vitest-environment node`). |
...
- **Every date agrees with the timeline.** The fixtures once ran in two camps a year apart (the timeline to Feb 2025; the digests, pivots, drift, last-seen and `DEMO_NOW` to Feb 2026, the same events a year later). `src/data/__tests__/dates.test.js` now holds every dated fixture to `TIMELINE_DATA`: a new dated fixture falls on or before `DEMO_NOW` (Feb 7 2025), and a topic's first- and last-seen come from its timeline.
- **Navigating on mount:** React Router starts listening in its own layout effect, which runs after its children's. A `navigate()` from a child's first layout effect updates the history but never re-renders the app, so the address bar and the screen disagree. Navigate from a mount `useEffect` (see the privacy respell in `useRouterSync`).
