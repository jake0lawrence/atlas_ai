...
const PastPerspectivePanel = ({ decisionId, mobile, onClose }) => {
  ...
  const outcomeColors = { positive: C.green, mixed: C.amber, negative: C.red };
  const outcomeIcons = { positive: "✓", mixed: "⚖", negative: "✗" };

  return (
    <div className="fade-up" style={{ marginTop: 12, background: `linear-gradient(135deg, ${alpha(C.purple, 0.06)}, ${alpha(C.purple, 0.02)}, ${alpha(C.bg0, 0.95)})`, border: `1px solid ${alpha(C.purple, 0.2)}`, borderRadius: 14, overflow: "hidden" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: mobile ? "12px 14px 8px" : "14px 20px 10px", borderBottom: `1px solid ${alpha(C.purple, 0.08)}` }}>
        <div style={row}>
      ...
      {analogy.analogies.map((a, i) => {
        ...
        return (
          <div key={i} style={{ padding: mobile ? "0 14px 14px" : "0 20px 16px", borderTop: i > 0 ? `1px solid ${alpha(C.purple, 0.06)}` : undefined, marginTop: i > 0 ? 8 : 0 }}>
