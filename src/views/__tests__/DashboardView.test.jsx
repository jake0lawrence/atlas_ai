...
import { TOPICS, INSIGHT_DECISIONS, RECURATION_COUNTS } from '../../data/constants';
...
describe('buildAttention', () => {
  it('pairs every stale decision with its topic and sorts unreviewed topics by count', () => {
    ...
    expect(a.stale.length).toBe(INSIGHT_DECISIONS.length);
...
describe('summarizeJourney', () => {
  ...
  it('matches the fixtures: Claude passed ChatGPT in Feb 24 and is 97% by Feb 26', () => {
    expect(summarizeJourney()).toEqual({ crossover: 'Feb 24', latestMonth: 'Feb 26', latestClaudeShare: 97 });
  });
});

describe('AttentionPanel', () => {
  it('shows three decisions, expands to all, and opens a topic from the new-conversations list', () => {
    ...
    fireEvent.click(within(panel).getByRole('button', { name: `Show all ${attention.stale.length}` }));
...
describe('DashboardView', () => {
  it('keeps the tour targets and states the journey takeaway in words', () => {
    ...
    expect(screen.getByText(/Claude passed ChatGPT in Feb 24\. By Feb 26 it was 97%/)).toBeTruthy();
